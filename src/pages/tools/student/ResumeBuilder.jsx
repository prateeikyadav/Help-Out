  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/gemini', {
        ...
        body: JSON.stringify({
          prompt: `Transform the following background information into a highly professional, ATS-optimized resume. Use clear markdown formatting with sections for Summary, Education, Experience, and Skills. Make it look impressive.\n\nRaw data:\n${input}`,
          systemInstruction: 'You are an elite technical recruiter and resume writer. Return ONLY the markdown formatted resume text, absolutely no conversational filler or introductory text.'
        })
      });
    ...
    <div className="flex-1 flex flex-col h-[calc(100vh-56px)] md:h-screen bg-bg overflow-hidden">
      <header className="bg-card border-b border-border px-6 py-4 flex items-center justify-between shrink-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-serif text-[22px] font-bold text-ink leading-none">AI Resume Builder</h1>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative" style={{ backgroundImage: 'radial-gradient(var(--color-borderMd) 1px, transparent 1px)', backgroundSize: '24px 24px' }}>
        ...
        <div className="w-full lg:w-1/2 h-full bg-card/90 backdrop-blur-md border-r border-borderMd flex flex-col relative z-10 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
          <div className="p-6 md:p-8 flex-1 flex flex-col">
            <label className="text-[13px] font-bold text-ink tracking-[0.05em] uppercase mb-2 block">Your Experience & Background</label>
            <p className="text-[13px] text-inkMid mb-4">Paste your raw notes, project details, or current resume. The AI will format and optimize it for ATS systems.</p>
            <textarea
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="E.g., I'm a 3rd year CS student. I built a React app for my portfolio and interned at a local startup doing data entry..."
          ...
          <div className="p-6 md:p-8 border-t border-borderMd bg-card">
            ...
            >
              {isGenerating ? <><Sparkles className="w-5 h-5 animate-pulse" /> Generating Resume...</> : <><Sparkles className="w-5 h-5" /> Generate ATS-Optimized Resume</>}
            </button>
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2 h-full overflow-y-auto p-6 md:p-12 flex items-start justify-center relative z-10">
          {!result ? (
            <div className="text-center mt-20 flex flex-col items-center opacity-50">
              <FileText className="w-16 h-16 text-inkLight mb-4 stroke-[1]" />
              <p className="text-[15px] text-inkMid font-medium">Your optimized resume will appear here.</p>
            </div>
          ) : (
